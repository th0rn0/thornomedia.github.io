...
| 2026-07-24 | Fixed the repeat bug for real — it wasn't the hold-length math, it was a silent runtime error plus a video/CSS clock mismatch | User reported the fix didn't actually work ("clips appear to be repeating sometimes"). Root-caused with a background sampler (`video.currentTime`/opacity logged every 300ms over 65s, 2+ full cycles) rather than eyeballing it: (1) `document.querySelectorAll<HTMLVideoElement>(...)` in the inline `<script>` — Astro doesn't strip TS syntax from client scripts the way it does frontmatter, so this was parsed as a chained JS comparison, silently evaluated to `false`, and every `.forEach` call on it threw. **The entire reduced-motion freeze and the first resync attempt never ran, on any page load.** Removed the generic. (2) Once actually running, a `setTimeout`/`setInterval` schedule computed from script-execution time still drifted from the CSS animation's own clock, because inline `<script>` runs during HTML parsing (before first paint) while CSS animations start counting from paint — different t=0 references. Replaced with `animationstart`/`animationiteration` event listeners, which tie the reset directly to the browser's own animation clock instead of a second independent timer. (3) Even then, the native `loop` attribute was still racing its own autoplay-driven clock against the controlled resets. Removed `loop` for the multi-clip case entirely — each clip now plays exactly once per visible appearance, driven solely by the animation events; if it ever runs out early it just freezes on its last frame, which happens to coincide with fade-out by construction. Verified with the same sampler: 0 currentTime regressions across 65s / 217 samples at any opacity threshold down to 0.5. Lesson: "no console errors" from a one-time post-load check isn't verification for code that runs on a timer — the browse tool's error capture apparently missed a synchronous throw at parse time; direct `eval` of the exact expression was what actually confirmed it. |
| 2026-07-24 | Removed the hold plateau — hero now continuously dissolves rather than pausing then fading | User: "they seem to pause then fade, I just want them to fade into each other." The hold-based model (however correctly implemented) always had several seconds of dead static time at full opacity between transitions. Redesigned: `FADE_i = duration_i / 2` per clip, no hold — each clip rises to a single instantaneous peak at its own midpoint then immediately falls, so at any moment ~2 clips are always blending. Fade speed now scales with each clip's own length (a 2s clip transitions in ~1s, a 7.5s clip over ~3.75s) rather than a shared constant — this also means each clip plays its full length exactly once per appearance with nothing left over to freeze on, reusing the same event-driven resync from the previous fix without changes. Verified via the sampler: opacity trace shows a clean rise-to-1.0-then-immediate-fall on every appearance, no plateau, 0 currentTime regressions across 65s/3 cycles. Expanded to 8 clips (2 more LanOps, 1 more Buhurt) per request — full cycle is now ~18.8s. |
| 2026-07-24 | Found and fixed a real reduced-motion bug: the hero went fully blank, not just static | User asked me to actually verify the reduced-motion path rather than assume it from code inspection, since the browse tool's CDP can't emulate `prefers-reduced-motion` (not on its allowlist). Launched `google-chrome-stable --headless=new --force-prefers-reduced-motion` directly instead, plus a raw CDP websocket connection (Node's native `WebSocket`, no library needed) to read actual `video.paused`/`currentTime`/computed-opacity state rather than trust a screenshot. Found `.hero-bg--animated:first-child { opacity: 1 }` never matched anything: the `{videos.length > 1 && <style set:html={...}>}` keyframes block renders as a literal DOM sibling *before* the first `<video>`, so that video is the CSS-tree's second child, not first — `:first-child` silently failed and every clip stayed at `opacity: 0`, ended a blank black hero for reduced-motion users (worse than the plain-static hero it was supposed to be). Fixed by switching to `:first-of-type`, which only counts `<video>` siblings and ignores the `<style>` tag. This is the second bug in this file found only by testing the actual runtime state instead of reading the code and reasoning it looked right — do the same for any future change to this component's reduced-motion path. |
| 2026-07-24 | Dropped the Buhurt sky/field clip, back to 7 clips | User's call, no reason given. Rotation is now overhead → lanops-tent → diagonal → buhurt-procession → lanops-hall → tree-tunnel → lanops-doorway, ~16.8s per full cycle. |
